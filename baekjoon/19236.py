                ...
                if matrix[r][c][0] == k: # find appropriate fish
                    ...
                    for i in range(8):
                        ...
                        if 0 <= rr < 4 and 0 <= cc < 4 and matrix[rr][cc][0] != 16:
                                matrix[r][c], matrix[rr][cc] = matrix[rr][cc], (fish, i)
                                break
                if c_flag:
                    break
            if r_flag:
    ...
    for i in range(1, 4):
        ...
        if 0 <= srr < 4 and 0 <= scc < 4:
            if matrix[srr][scc][0] < 16:
                inner_matrix = copy.deepcopy(matrix) # copy the original matrix
                fish, direction = matrix[srr][scc] # extract fish and direction
                inner_matrix[sr][sc], inner_matrix[srr][scc] = (20, 20), (16, direction) # update
                ...
                # print()
                dq.append((inner_matrix, shark_inner_count)) # append new matrix and updated shark count
                results.append(shark_inner_count) # append shark count
    # print(dq)
