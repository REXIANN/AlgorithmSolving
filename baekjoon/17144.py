...
for test_count in range(tc):
    ...
    ap_r, ap_r2 = tuple(i for i in range(R) if matrix[i][0]) #ap_r, ap_rr 은 공기청정기가 있는 행
    # pprint(matrix)
    sum_ = 0
    dr = [-1, 0, 1, 0, 1, 0, -1, 0]
    dc = [0, 1, 0, -1, 0, 1, 0, -1]
    ...
    for h in range(T):
            ...
            for j in range(C):
                ...
                for k in range(4):
                    if 0 <= i+dr[k] < R and 0 <= j+dc[k] < C and matrix[i+dr[k]][j+dc[k]] != -1:
                        ...
                        matr[i][j] -= val    
        # print(f'{h+1} les poussieres'); pprint(matr)

        # upper part of air purificator 
        r, c , k = ap_r - 1, 0, 0
        while k < 4:
        ...
        # print(f'{h} purifier'); pprint(matr)
